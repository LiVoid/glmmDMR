...
# Modified version with bug fixes (2026-02-04)
...
usage <- function() {
  ...
  Options:
    ...
    -g <int>      Max gap (bp) to merge nearby DMRs with same direction
            into a merged BED (def. 200)
...
mergeGap <- 200        # max gap to merge nearby DMRs (bp)
...
# FIX: Sort data by chr and position to avoid DSS warnings
if (verbose) {
  cat('Sorting data by chromosome and position\n')
}
...
# FIX: Check if we have any results after filtering
if (nrow(res) == 0) {
  if (verbose) {
    cat('Warning: No regions passed filtering criteria.\n')
  }
  write.table(res, output, sep='\t', quote=F, row.names=F)
  if (verbose) {
    cat('Regions reported: 0\n')
  }
  quit()
}
...
# FIX: sort chromosome names by number/letter
# Ensure chr is a factor
if (!is.factor(res$chr)) {
  res$chr <- as.factor(res$chr)
}
level <- levels(res$chr)

# FIX: Check if we have any levels to sort
if (length(level) == 0) {
  if (verbose) {
    cat('Warning: No chromosome levels found. Using unsorted output.\n')
  }
  write.table(res, output, sep='\t', quote=F, row.names=F)
  if (verbose) {
...
# FIX: Use seq_along instead of 1:length to avoid empty vector issues
...
res_out <- res[order(match(res$chr, chrOrder), res$start), ]
write.table(res_out, output, sep='\t', quote=F, row.names=F)

# optional merged BED output (same direction, within mergeGap)
merged_bed <- paste0(outfile, '_merged.bed')
if (nrow(res_out) > 0) {
  diff_cols <- grep('(:diff$|^diff$)', colnames(res_out), value=TRUE)
  if (length(diff_cols) > 0) {
    diff_mat <- as.matrix(res_out[, diff_cols, drop=FALSE])
    mean_diff <- rowMeans(diff_mat, na.rm=TRUE)
    res_out$direction <- ifelse(mean_diff >= 0, 'hyper', 'hypo')

    res_out <- res_out[order(match(res_out$chr, chrOrder), res_out$start), ]
    merged <- list()
    i <- 1
    while (i <= nrow(res_out)) {
      cur_chr <- res_out$chr[i]
      cur_dir <- res_out$direction[i]
      cur_start <- res_out$start[i]
      cur_end <- res_out$end[i]
      j <- i + 1
      while (j <= nrow(res_out) && res_out$chr[j] == cur_chr && res_out$direction[j] == cur_dir) {
        gap <- res_out$start[j] - cur_end
        if (gap <= mergeGap) {
          cur_end <- max(cur_end, res_out$end[j])
          j <- j + 1
        } else {
          break
        }
      ...
      merged[[length(merged) + 1]] <- data.frame(
        chr = as.character(cur_chr),
        start = as.integer(cur_start - 1L),
        end = as.integer(cur_end),
        direction = as.character(cur_dir),
        stringsAsFactors = FALSE
      )
      i <- j
    }
    merged_df <- do.call(rbind, merged)
    write.table(merged_df, merged_bed, sep='\t', quote=F, row.names=F, col.names=FALSE)
  } else if (verbose) {
    cat('Warning: No diff columns found; merged BED not written.\n')
  }
} else if (verbose) {
  cat('Warning: No DMRs; merged BED not written.\n')
}
if (verbose) {
  cat('Regions reported:', nrow(res), '\n')
}
